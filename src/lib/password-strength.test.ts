...
describe("scorePassword", () => {
  ...
  describe("weak passwords (should NOT pass)", () => {
    const weakPasswords: Array<{ pw: string; reason: string }> = [
      ...
      { pw: "Abcdefg1", reason: "8 chars + upper + lower + digit = 4, but no 12+ and no special → 4 (borderline, see below)" },
    ];
  ...
  describe("strong passwords (should pass)", () => {
    const strongPasswords: Array<{ pw: string; reason: string }> = [
      { pw: "Abcdefg1!", reason: "8 chars + upper + lower + digit + special = 5" },
      { pw: "StrongPass1!", reason: "12 chars + upper + lower + digit + special = 6" },
      { pw: "MyP@ssw0rd2024", reason: "14 chars + upper + lower + digit + special = 6" },
      { pw: "Abcdefghijkl1", reason: "13 chars + upper + lower + digit = 5 (no special but 12+)" },
    ];
  ...
  describe("labels", () => {
    ...
    it("returns 'Fair' for score 3", () => {
      expect(scorePassword("Abcdefgh").label).toBe("Fair"); // 8+ upper lower = 3
    });

    it("returns 'Good' for score 4", () => {
      expect(scorePassword("Abcdefg1").label).toBe("Good"); // 4
    });

    it("returns 'Strong' for score >= 5", () => {
      expect(scorePassword("Abcdefg1!").label).toBe("Strong"); // 5
  ...
  describe("passes flag", () => {
    it("is false for scores below MIN_PASSWORD_SCORE", () => {
      ...
      expect(scorePassword("Abcdefgh").passes).toBe(false); // score 3
    });

    it("is true for scores at or above MIN_PASSWORD_SCORE", () => {
      expect(scorePassword("Abcdefg1").passes).toBe(true); // score 4
      expect(scorePassword("Abcdefg1!").passes).toBe(true); // score 5
