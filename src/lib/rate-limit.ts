...
export async function rateLimit(
  ...
  try {
    ...
    // SECURITY: Fail CLOSED for sensitive presets (login, register, otp)
    // to prevent brute-force attacks during Upstash outages. Fail OPEN
    // for less-sensitive presets (scan, api) to avoid blocking normal use.
    const failClosed = preset === "login" || preset === "register" || preset === "otp";
    console.error(`[rate-limit] Upstash error, failing ${failClosed ? "CLOSED" : "open"}:`, e);
    if (failClosed) {
