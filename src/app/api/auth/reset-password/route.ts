...
export async function POST(req: NextRequest) {
  try {
    ...
    // Single-use enforcement: the recovery session is consumed by the
    // password update. Sign it out so a captured/stolen recovery session
    // cannot be replayed to reset the password again. Per 06-security-
    // architecture.md §2, a recovery flow must be usable exactly once.
