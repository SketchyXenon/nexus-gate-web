...
import {
  ShieldAlert, Search, Ban, CheckCircle2, ChevronLeft, ChevronRight,
  Plus, Trash2, UserPlus, Loader2, Mail, Lock, Hash,
  Pencil, ShieldCheck,
} from "lucide-react";
import {
...
import {
  useAccounts, useUpdateAccount, useAdminCreateAccount, useDeleteAccount,
  type Account,
} from "@/lib/api-client";
...
export function AccountsView({ currentUser }: { currentUser?: Account }) {
  ...
  const accounts = rawAccounts
    ...
    .sort((a, b) => {
      switch (sortBy) {
        ...
        case "lastlogin-desc": return new Date(b.lastLoginAt || 0).getTime() - new Date(a.lastLoginAt || 0).getTime();
        default: return new Date(b.createdAt || 0).getTime() - new Date(a.createdAt || 0).getTime(); // created-desc
      }
    });
  ...
  async function confirmAction() {
    ...
    await updateMut.mutateAsync(
      { id: account.id, status: value },
      {
        onSuccess: () => {
          toast({
            title: value === "SUSPENDED" ? "Account suspended" : "Account reactivated",
            description:
              value === "SUSPENDED"
        ...
        onError: (e) =>
          toast({ title: "Couldn't update account", description: e.message, variant: "destructive" }),
      },
    );
  }
  ...
  async function saveEdit() {
    ...
    const { account, fullName, email, program, section, year, role, status, organizationName } = editTarget;
    await updateMut.mutateAsync(
      {
        id: account.id,
        ...
        organizationName: role === "ORGANIZER" ? organizationName.trim() || null : null,
      },
      {
        onSuccess: () => {
          toast({
  ...
  function handleCreate(e: React.FormEvent) {
    ...
    if (newPassword.length < 8) errs.password = "Min 8 characters";
    ...
    createMut.mutate(
      {
        ...
        organizationName: newRole === "ORGANIZER" ? (newOrgName.trim() || undefined) : undefined,
      },
      {
        onSuccess: () => {
          toast({ title: "Account created", description: `${newFullName} can now sign in.` });
          setCreateOpen(false);
          setNewEmail(""); setNewPassword(""); setNewFullName("");
          setNewRole("ORGANIZER"); setNewProgram(""); setNewSection(""); setNewOrgName("");
          setCreateErrors({});
        },
        onError: (e) =>
  ...
  return (
    <div className="space-y-6">
        ...
        <CardHeader>
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              ...
              <CardDescription>
                {pagination?.total ?? 0} {pagination?.total === 1 ? "account" : "accounts"} total
              </CardDescription>
            </div>
        ...
        <CardContent className="p-0">
          {isError && (
            <div className="px-6 pb-4">
              <p className="text-sm text-destructive">Couldn't load accounts. Please try again.</p>
            </div>
          )}
              ...
              <TableBody>
                ...
                {accounts.map((a, idx) => (
                  ...
                  >
                    ...
                    <TableCell className="hidden sm:table-cell text-xs text-muted-foreground">{a.email}</TableCell>
                    <TableCell>
                      <Badge variant="outline" className={
                        a.role === "ADMIN" ? "border-primary/40 text-primary"
                        : a.role === "ORGANIZER" ? "border-amber-500/40 text-amber-600"
                    ...
                    <TableCell>
                      <Badge variant="outline" className={
                        a.status === "ACTIVE" ? "border-emerald-500/40 text-emerald-600"
                        : a.status === "SUSPENDED" ? "border-red-500/40 text-red-600"
                      ...
                      {a.program ? (
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <Badge variant="outline" className="cursor-help text-[11px]">{a.program}</Badge>
                          </TooltipTrigger>
                          <TooltipContent>{getProgramLabel(a.program) ?? a.program}</TooltipContent>
                        </Tooltip>
                      ) : (
                    ...
                    <TableCell className="hidden lg:table-cell text-xs text-muted-foreground">
                      {a.lastLoginAt ? format(new Date(a.lastLoginAt), "MMM d, HH:mm") : "Never"}
                    </TableCell>
                          ...
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button
                                ...
                                onClick={() => setPendingAction({ account: a, value: "SUSPENDED" })}
                              >
                          ...
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button
                                ...
                                onClick={() => setPendingAction({ account: a, value: "ACTIVE" })}
                              >
                          ...
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button
                                ...
                                onClick={() => setDeleteTarget({ account: a, hard: true })}
                              >
          ...
          {pagination && (
            <div className="flex items-center justify-between px-4 py-3 border-t gap-2 flex-wrap">
              <p className="text-xs text-muted-foreground">
                Page {pagination.page} of {pagination.totalPages} · {pagination.total} accounts total
              </p>
              <div className="flex gap-2 items-center">
                ...
                <span className="text-xs font-medium tabular-nums min-w-[2rem] text-center" aria-label={`Page ${pagination.page} of ${pagination.totalPages}`}>
                  {pagination.page}
                </span>
      ...
      <Dialog open={createOpen} onOpenChange={(o) => { setCreateOpen(o); if (!o) setCreateErrors({}); }}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Create an administrator or organizer account. Students register themselves.
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleCreate} className="space-y-3" noValidate>
            <div className="space-y-1.5">
              <Label htmlFor="newFullName">Full name</Label>
              <Input id="newFullName" placeholder="Jane Smith" value={newFullName}
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <Input id="newEmail" type="email" placeholder="name@ctu.edu.ph" value={newEmail}
                  onChange={(e) => setNewEmail(e.target.value)} className="pl-9" />
              </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <Input id="newPassword" type="password" placeholder="••••••••" value={newPassword}
                  onChange={(e) => setNewPassword(e.target.value)} className="pl-9" />
              </div>
              ...
              <>
                <div className="grid grid-cols-2 gap-2">
                  <div className="space-y-1.5">
                    ...
                    <Input id="newProgram" placeholder="BSIT" value={newProgram}
                      onChange={(e) => setNewProgram(e.target.value)} />
                  </div>
                  <div className="space-y-1.5">
                    <Label htmlFor="newSection">Section (optional)</Label>
                    <Input id="newSection" placeholder="2-B" value={newSection}
                      onChange={(e) => setNewSection(e.target.value)} />
                  </div>
                </div>
                <div className="space-y-1.5">
                  <Label htmlFor="newOrgName">Organization name (optional)</Label>
                  <Input id="newOrgName" placeholder="e.g. College of Technology" value={newOrgName}
      ...
      <ConfirmDialog
        ...
        title={pendingAction?.value === "SUSPENDED" ? "Suspend this account?" : "Reactivate this account?"}
        description={
        ...
        confirmLabel={pendingAction?.value === "SUSPENDED" ? "Suspend account" : "Reactivate account"}
        confirmText={pendingAction?.value === "SUSPENDED" ? "SUSPEND" : "ACTIVATE"}
        step2Warning={
          pendingAction?.value === "SUSPENDED"
      ...
      <Dialog open={!!editTarget} onOpenChange={(o) => !o && setEditTarget(null)}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Update {editTarget?.account?.fullName ?? ""}'s details. Changes are saved immediately.
            </DialogDescription>
          </DialogHeader>
          {editTarget && (
            <div className="space-y-3">
              <div className="space-y-1.5">
                ...
                <Input id="editFullName" value={editTarget.fullName}
                  onChange={(e) => setEditTarget({ ...editTarget, fullName: e.target.value })} />
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="editEmail">Email</Label>
                <Input id="editEmail" type="email" value={editTarget.email}
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="space-y-1.5">
                  ...
                  <Select value={editTarget.role}
                    onValueChange={(v) => setEditTarget({ ...editTarget, role: v as Account["role"] })}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="USER">Student</SelectItem>
                      <SelectItem value="ORGANIZER">Organizer</SelectItem>
                ...
                <div className="space-y-1.5">
                  ...
                  <Select value={editTarget.status}
                    onValueChange={(v) => setEditTarget({ ...editTarget, status: v as Account["status"] })}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="ACTIVE">Active</SelectItem>
                      <SelectItem value="SUSPENDED">Suspended</SelectItem>
              ...
              <div className="space-y-1.5">
                <Label>Program</Label>
                <Select value={editTarget.program}
                  onValueChange={(v) => setEditTarget({ ...editTarget, program: v })}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="space-y-1.5">
                  ...
                  <Input id="editSection" value={editTarget.section}
                ...
                {editTarget.role === "USER" && (
                  <div className="space-y-1.5">
                    ...
                    <Select value={String(editTarget.year)}
                      onValueChange={(v) => setEditTarget({ ...editTarget, year: Number(v) })}>
                      <SelectTrigger><SelectValue /></SelectTrigger>
                      <SelectContent>
                        {[1, 2, 3, 4, 5, 6].map((y) => (
              ...
              {editTarget.role === "ORGANIZER" && (
                <div className="space-y-1.5">
                  ...
                  <Input id="editOrgName" value={editTarget.organizationName}
                    onChange={(e) => setEditTarget({ ...editTarget, organizationName: e.target.value })}
