...
import {
  User, Mail, Lock, Clock, Save, Loader2, KeyRound, Shield,
  GraduationCap, Calendar, Eye, EyeOff, AlertCircle, Info,
} from "lucide-react";
import {
...
import { useProfile, useUpdateProfile, useChangePassword } from "@/lib/api-client";
...
export function ProfileView() {
  ...
  function handleConfirmSave() {
    ...
    const vars: { fullName: string; program?: string; year?: number; section?: string } = {
    ...
    <div className="max-w-2xl mx-auto space-y-6">
          ...
          <CardContent className="p-6">
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                <h2 className="font-heading text-xl font-bold truncate">{profile.fullName}</h2>
                <p className="text-sm text-muted-foreground truncate">{profile.email}</p>
                <div className="flex gap-2 mt-2">
                  <Badge variant="secondary">{ROLE_LABELS[profile.role]}</Badge>
                  <Badge variant="outline" className={
                    profile.status === "ACTIVE" ? "border-emerald-500/40 text-emerald-600" :
                    profile.status === "SUSPENDED" ? "border-red-500/40 text-red-600" :
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          {profile.studentId && (
            <InfoRow icon={<GraduationCap className="h-4 w-4" />} label="Student ID" value={String(profile.studentId)} />
          )}
          {profile.program && (
            <InfoRow icon={<User className="h-4 w-4" />} label="Program" value={profile.program} />
          )}
          {profile.section && (
            <InfoRow icon={<User className="h-4 w-4" />} label="Section" value={profile.section} />
          )}
          {profile.year && (
            <InfoRow icon={<GraduationCap className="h-4 w-4" />} label="Year" value={`Year ${profile.year}`} />
          )}
          <InfoRow icon={<Calendar className="h-4 w-4" />} label="Member since" value={format(new Date(profile.createdAt || Date.now()), "MMMM d, yyyy")} />
          {profile.lastLoginAt && (
            <InfoRow icon={<Clock className="h-4 w-4" />} label="Last sign-in" value={format(new Date(profile.lastLoginAt), "MMM d, yyyy 'at' HH:mm")} />
          )}
        </CardContent>
      </Card>
            ...
            <CardDescription className="flex items-center gap-1.5">
              ...
              {isUser && profile.canUpdateProfile && (
                <Tooltip>
                  ...
                  <TooltipContent>Profile updates are limited to once every 30 days. Course can only be changed once.</TooltipContent>
                </Tooltip>
              )}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
                      ...
                      <Label htmlFor="program" className="flex items-center gap-1.5">
                        ...
                        {!profile.canChangeCourse && (
                          <Tooltip>
                            ...
                            <TooltipContent>Locked — course can only be changed once. Contact an admin to reset.</TooltipContent>
                          </Tooltip>
                        )}
                      </Label>
                      <Select
                        ...
                        disabled={!profile.canUpdateProfile || !profile.canChangeCourse || updateProfile.isPending}
                      >
                  ...
                  <div className="space-y-1.5">
                    <Label htmlFor="section" className="flex items-center gap-1.5">
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent>Format: year-letter (e.g. "2-A", "3-B"). Must match your year level.</TooltipContent>
                      </Tooltip>
                    </Label>
                    <Input
                      id="section"
              ...
              >
                ...
                {profile.canUpdateProfile ? "Save profile" : `Locked for ${profile.daysUntilProfileUpdate} more days`}
              </Button>
            </div>
          </CardContent>
        </Card>
          ...
          <CardDescription className="flex items-center gap-1.5">
            ...
            {!profile.canChangePassword && (
              <Tooltip>
                ...
                <TooltipContent>Password changes are limited to once every 30 days for security.</TooltipContent>
              </Tooltip>
            )}
          </CardDescription>
        </CardHeader>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              You can only update your profile once every 30 days. Please review your changes carefully before saving.
              {isUser && profile.canChangeCourse && (program === NO_PROGRAM ? "" : program) !== (profile.program ?? "") && (
                <span className="block mt-2 text-amber-600 font-medium">
                  Warning: You are changing your course from "{profile.program || "— Not specified —"}" to "{program === NO_PROGRAM ? "— Not specified —" : program}". This can only be done once.
                </span>
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
          ...
          <DialogHeader>
            ...
            <DialogDescription className="flex items-center gap-1.5">
              ...
              <Tooltip>
                ...
                <TooltipContent>You'll be signed out on other devices. Password changes are limited to once every 30 days.</TooltipContent>
              </Tooltip>
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleChangePassword} className="space-y-3">
            ...
            <div className="space-y-1.5">
              <Label htmlFor="newPassword" className="flex items-center gap-1.5">
                ...
                <Tooltip>
                  ...
                  <TooltipContent>Must be "Good" or "Strong" — 8+ chars with uppercase, lowercase, a number, and a special character (or 12+ chars).</TooltipContent>
                </Tooltip>
              </Label>
              <div className="relative">
                <Input
            ...
            <div className="flex gap-2 pt-2">
              ...
              <Button
                type="submit"
                className="flex-1"
                disabled={changePassword.isPending || (newPassword.length > 0 && !scorePassword(newPassword).passes)}
              >
